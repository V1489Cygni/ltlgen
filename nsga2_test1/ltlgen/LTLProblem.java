...
public class LTLProblem extends GPProblem {
    ...
    public void setup(final EvolutionState state, final Parameter base) {
        ...
        for (int i = mutatedNumber; i < testAutomatons.length; i++) {
            try {
                Process p = Runtime.getRuntime().exec("./generate-efsm -n " + stateNumber + " -e " + EVENT_NUMBER + " -a " + ACTION_NUMBER + " -m 0 -s 2 -v 1");
    ...
    public void evaluate(EvolutionState state, Individual ind, int subpopulation, int threadnum) {
        if (!ind.evaluated) {
            ...
            f.setObjectives(state, new double[]{1 / (1.0 + result), 1 / (1.0 + input.size)});
            ind.evaluated = true;
        }
    }
}